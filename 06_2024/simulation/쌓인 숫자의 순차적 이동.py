...
def move_to_max_neighbor(row, col, target):
    ...
    if (
        row > 0
        and col > 0
        and len(mat[row - 1][col - 1])
        and max(mat[row - 1][col - 1]) >= region_max
    ):  # upper left
        region_max = max(mat[row - 1][col - 1])
        targ_row, targ_col = row - 1, col - 1
    if (
        row > 0 and len(mat[row - 1][col]) and max(mat[row - 1][col]) >= region_max
    ):  # upper middle
    ...
    if (
        ...
        targ_row, targ_col = row - 1, col + 1
    if (
        col < n - 1 and len(mat[row][col + 1]) and max(mat[row][col + 1]) >= region_max
    ):  # right middle
    ...
    if (
        ...
        targ_row, targ_col = row + 1, col + 1
    if (
        row < n - 1 and len(mat[row + 1][col]) and max(mat[row + 1][col]) >= region_max
    ):  # bottom middle
        region_max = max(mat[row + 1][col])
        targ_row, targ_col = row + 1, col
    if (
        ...
        targ_row, targ_col = row + 1, col - 1
    if (
        col > 0 and len(mat[row][col - 1]) and max(mat[row][col - 1]) >= region_max
    ):  # left middle
        region_max = max(mat[row][col - 1])
        targ_row, targ_col = row, col - 1
