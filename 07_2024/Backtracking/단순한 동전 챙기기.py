...
def try_path(coin_cnt, move_cnt, idx):
    ...
    if coin_cnt >= 3:
        move_cnt = travel(
            coin_locations[coins[idx]][0],
            coin_locations[coins[idx]][1],
            e_row,
            e_col,
            move_cnt,
        )
    ...
    for i in range(idx + 1, coin_len):
        new_move_cnt = travel(
            coin_locations[coins[idx]][0],
            coin_locations[coins[idx]][1],
            coin_locations[coins[i]][0],
            coin_locations[coins[i]][1],
            move_cnt,
        )
...
for i in range(len(coins)):
    try_path(
        1,
        travel(
            s_row, s_col, coin_locations[coins[i]][0], coin_locations[coins[i]][1], 0
        ),
        i,
    )
print(ans)

# 굳이 딕셔너리 쓰지 말고 2차원 배열 순회하면서 동전 숫자 순서대로 리스트 append하면 된다. 그러면 훨씬 코드가 간단해질듯.
# 최단 거리 그냥 abs로 두 점 사이 거리 구하면 된다.
